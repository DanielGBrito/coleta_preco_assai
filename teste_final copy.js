...
(async () => {
  ...
  try {
    ...
    response = await httpGet(URL_PRODUTOS, headersLimpos, tlsAgent);
  } catch (err) {
    if (!isSslCertificateError(err)) {
      throw err;
    }

    console.error('Erro SSL ao consultar produtos.');
    console.error('Se sua rede usa proxy/certificado corporativo, configure IFOOD_CA_BUNDLE com o caminho do .pem.');
    console.error("Exemplo PowerShell: $env:IFOOD_CA_BUNDLE='C:\\certs\\empresa-root-ca.pem'");
    console.error("Apenas para teste (inseguro): $env:IFOOD_INSECURE_SSL='1'");

    const insecureJaHabilitado = ['1', 'true', 'yes'].includes(String(process.env.IFOOD_INSECURE_SSL || '').toLowerCase());
    const autoFallback = ['1', 'true', 'yes'].includes(String(process.env.IFOOD_AUTO_FALLBACK_INSECURE_SSL || '1').toLowerCase());

    if (!insecureJaHabilitado && autoFallback) {
      console.warn('Tentando fallback automatico em modo inseguro para concluir a consulta...');
      ...
      response = await httpGet(URL_PRODUTOS, headersLimpos, insecureAgent);
      console.warn('Fallback inseguro aplicado com sucesso. Configure IFOOD_CA_BUNDLE para usar modo seguro.');
    } else {
      throw err;
    }
  }
